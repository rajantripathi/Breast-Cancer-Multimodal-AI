#!/usr/bin/env bash
#SBATCH --job-name=bcai-sf-giga-mean
...
SEED="${SEED:-42}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/stage2/gigapath/simple_fusion_mean/seed${SEED}}"
cd "$REPO_DIR"
mkdir -p "$OUTPUT_DIR"
...
python -u -m training.tcga_simple_fusion \
  --crosswalk data/tcga_crosswalk_gigapath_pathways.csv \
