#!/bin/bash
# Train the non-legacy screener on VinDr with CBIS-DDSM as a train-only auxiliary source.
# VinDr val/test remain the benchmark splits; CBIS contributes to training only.
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/mammography/vindr_cbis_aux_seed42_rerun}"
SEED="${SEED:-42}"
