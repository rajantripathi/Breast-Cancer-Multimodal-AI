...
# as a train-only auxiliary source. VinDr val/test remain the benchmark splits.
# This launcher uses explicit equal source weights so CBIS stays an opt-in
# ablation rather than dominating via inverse-frequency reweighting.

#SBATCH --job-name=bcai-mammo-vcaux
...
#SBATCH --output=logs/mammo_vcaux_%j.out
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/mammography/vindr_cbis_equal_weight_seed42}"
SEED="${SEED:-42}"
