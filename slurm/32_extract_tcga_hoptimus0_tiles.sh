#!/usr/bin/env bash
#SBATCH --job-name=bcai-hoptimus-extract
...
#SBATCH --output=logs/hoptimus_extract_%A_%a.out
#SBATCH --array=0-0
...
if [ -z "${HF_TOKEN:-}" ]; then
  echo "HF_TOKEN must be set in the environment or sbatch --export" >&2
  exit 1
fi
...
"$VENV_DIR/bin/python" -u -m data.preprocess.extract_tcga_features \
  ...
  --tile-list "$TILE_LIST" \
  --output-dir "$PROJECT_ROOT/tcga-brca/embeddings/hoptimus0" \
  --patch-output-dir "$PROJECT_ROOT/tcga-brca/patch_embeddings/hoptimus0"
