#!/usr/bin/env bash
#SBATCH --job-name=bcai-ctrans-extract
...
#SBATCH --output=logs/ctrans_extract_%A_%a.out
#SBATCH --array=0-0
...
"$VENV_DIR/bin/python" -u -m data.preprocess.extract_tcga_features \
  ...
  --tile-list "$TILE_LIST" \
  --output-dir "$PROJECT_ROOT/tcga-brca/embeddings/ctranspath" \
  --patch-output-dir "$PROJECT_ROOT/tcga-brca/patch_embeddings/ctranspath"
