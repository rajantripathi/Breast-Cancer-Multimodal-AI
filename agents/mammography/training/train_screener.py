...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["none", "source_percentile"],
        default="none",
        help="Optional train-fit source-aware intensity harmonization.",
    )
    parser.add_argument(
        "--harmonization-stats-json",
        default=None,
        help="Optional harmonization stats JSON. If omitted and harmonization is enabled, stats are fit on the training split.",
    )
    parser.add_argument("--harmonization-lower-quantile", type=float, default=0.01)
    parser.add_argument("--harmonization-upper-quantile", type=float, default=0.99)
    parser.add_argument("--harmonization-max-images-per-source", type=int, default=256)
...
class MammographyExamDataset(Dataset):
    def __init__(
        ...
        self.harmonization_stats = harmonization_stats
    ...
    def _load_view(self, exam, view_key):
        ...
        arr = apply_source_harmonization(arr, exam.get("dataset_source"), self.harmonization_stats)
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "dataset_source": str(exam.get("dataset_source") or "unknown"),
        }
...
def collate_batch(batch):
    ...
    return {"views": views, "labels": labels, "study_ids": study_ids, "dataset_sources": dataset_sources}
...
@torch.no_grad()
def predict_dataset(model, loader, device, tta="none"):
    model.eval()
    predictions = []
    for batch in loader:
        views = {k: v.to(device) for k, v in batch["views"].items()}
...
def main():
    ...
    harmonization_stats = None
    harmonization_config = {"method": "none"}
    if args.harmonization_stats_json:
        ...
        harmonization_config = {
            "method": str(harmonization_stats.get("method") or args.harmonization_method),
            "stats_json": str(Path(args.harmonization_stats_json)),
            "sources": sorted((harmonization_stats.get("sources") or {}).keys()),
        }
        save_harmonization_stats(output_dir / "harmonization_stats.json", harmonization_stats)
    elif args.harmonization_method != "none":
        harmonization_stats = fit_source_harmonization(
            split_buckets["train"],
            lower_quantile=float(args.harmonization_lower_quantile),
            upper_quantile=float(args.harmonization_upper_quantile),
            max_images_per_source=int(args.harmonization_max_images_per_source),
        )
        harmonization_config = {
            "method": str(harmonization_stats.get("method") or args.harmonization_method),
            "lower_quantile": float(args.harmonization_lower_quantile),
        ...
        save_harmonization_stats(output_dir / "harmonization_stats.json", harmonization_stats)
        print(f"Harmonization sources: {harmonization_config['sources']}")
    ...
    val_ds = MammographyExamDataset(
        split_buckets["val"],
        args.image_size,
        training=False,
        harmonization_stats=harmonization_stats,
    )
    test_ds = MammographyExamDataset(
        split_buckets["test"],
        args.image_size,
        training=False,
        harmonization_stats=harmonization_stats,
    ...
    test_predictions = predict_dataset(model, test_loader, device, tta=args.tta)
    ...
    summary = {
        ...
        "source_weights": train_source_weights,
        "harmonization": harmonization_config,
        "git_commit": get_git_commit(),
    }
